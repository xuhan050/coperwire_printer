...
public:
  ...
  #define LETTER_BIT(N) PARAM_BIT(LETTER_OFF(N))
    ...
    // This allows "if (seen('A')||seen('B'))" to use the last-found value
    // seen('A') if A is in teh string, return true.
    static bool seen(const char c) {
  ...
  inline static float value_float() {
    if (value_ptr) {
      ...
      return strtod(value_ptr, NULL); // strtod:tranform string into double
    }
    return 0.0;
  }
  ...
  // Units modes: Inches, Fahrenheit, Kelvin
    ...
    FORCE_INLINE static float value_axis_units(const AxisEnum a)    { UNUSED(a); return value_float(); }// get the position value of a axis
