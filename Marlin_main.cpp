...
float current_position_AD[XYZE_AD] = {0.0}; //HANXU:for H
...
float destination_AD[XYZE_AD] = { 0.0 }; //HANXU:FOR H
...
bool axis_relative_modes_AD[] = AXIS_RELATIVE_MODES_AD; //HANXU: for H
...
const char axis_codes_AD[XYZE_AD] = {'X', 'Y', 'Z', 'E', 'H'}; //HANXU: E1 is labeled as 'H'
...
void prepare_move_to_destination_AD(); //HANXU: for H
...
void sync_plan_position() {
  ...
  planner.set_position_mm_AD(current_position_AD[X_AXIS], current_position_AD[Y_AXIS], current_position_AD[Z_AXIS], current_position_AD[E_AXIS], current_position_AD[H_AXIS]); //HANXU
}
inline void sync_plan_position_e() { planner.set_e_position_mm(current_position[E_AXIS]); }
  ...
  #define SYNC_PLAN_POSITION_KINEMATIC() sync_plan_position() //what is running
...
inline void line_to_destination_AD(const float fr_mm_s) {
  planner.buffer_line_AD(destination_AD[X_AXIS], destination_AD[Y_AXIS], destination_AD[Z_AXIS], destination_AD[E_AXIS], destination_AD[H_AXIS], fr_mm_s, active_extruder);
} 
inline void line_to_destination_AD() { line_to_destination_AD(feedrate_mm_s); }//HANXU: two inline functions
...
inline void set_current_to_destination_AD() { COPY(current_position_AD, destination_AD); }
...
static void homeaxis(const AxisEnum axis) {
  ...
  if (bump) {
	...
	destination_AD[axis]=destination[axis];
	current_position_AD[axis]=current_position[axis];
...
//HANXU: for H
void gcode_get_destination_AD() {
  LOOP_XYZE_AD(i) {
    if (parser.seen(axis_codes_AD[i]))// HANXU: for H
      destination_AD[i] = parser.value_axis_units((AxisEnum)i) + (axis_relative_modes_AD[i] || relative_mode ? current_position_AD[i] : 0);//HANXU: for H
    else
      destination_AD[i] = current_position_AD[i];// HANXU: for H; the function is to get the value for destination_AD
  }

  if (parser.linearval('F') > 0.0)
    feedrate_mm_s = MMM_TO_MMS(parser.value_feedrate()); //get feedrate
...
// HANXU: for H
inline void gcode_G0_G1_AD(
  #if IS_SCARA
    bool fast_move=false
  #endif
) {
  if (IsRunning()) {
    gcode_get_destination_AD(); // For X Y Z E H F
      ...
      fast_move ? prepare_uninterpolated_move_to_destination() : prepare_move_to_destination();
    #else
      prepare_move_to_destination_AD(); //HANXU: for H
    #endif
  }
}
...
inline void gcode_G28(const bool always_home_all) {
  ...
  pinMode(44, OUTPUT);
  analogWrite(44, 0);//Han Xu

  destination_AD[H_AXIS] = current_position_AD[H_AXIS] = 0.0;
  destination [E_AXIS] = current_position[E_AXIS] = destination_AD [E_AXIS] = current_position_AD[E_AXIS] = 0.0;// HANXU: for H
...
inline void gcode_M42() {
  ...
  //digitalWrite(pin_number, pin_status);mark_1_26
...
void process_next_command() {
  ...
  switch (parser.command_letter) {
    case 'G': switch (parser.codenum) {
      ...
      case 0:
	  	#if IS_SCARA
          gcode_G0_G1_AD(parser.codenum == 0);
        #else
          gcode_G0_G1_AD();  //HANXU:control value
        #endif
        break;
		
	  //G1 for also fro E_AD
      case 1:
        #if IS_SCARA
          gcode_G0_G1_AD(parser.codenum == 0);
        #else
      ...
      case 90: // G90
        ...
        break;
      case 91: // G91
        relative_mode = false; // HANXU:G91 should have been the relative_mode. I disable it for safety reason.
        break;
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
  ...
  inline bool prepare_move_to_destination_cartesian() {
      ...
      if (current_position[X_AXIS] == destination[X_AXIS] && current_position[Y_AXIS] == destination[Y_AXIS])
        ...
        const float fr_scaled = MMS_SCALED(feedrate_mm_s); // feedrate is the maximum speed
        #if ENABLED(MESH_BED_LEVELING)
...
inline bool prepare_move_to_destination_cartesian_AD() {
  #if ENABLED(AUTO_BED_LEVELING_UBL)
	const float fr_scaled = MMS_SCALED(feedrate_mm_s);
	if (ubl.state.active) { // direct use of ubl.state.active for speed
	  ubl.line_to_destination_cartesian(fr_scaled, active_extruder);
  ...
  #else
	// Do not use feedrate_percentage for E or Z only moves
	if (current_position_AD[X_AXIS] == destination_AD[X_AXIS] && current_position_AD[Y_AXIS] == destination_AD[Y_AXIS])
	  line_to_destination_AD();//HANXU: set in to buff
	else {
	  const float fr_scaled = MMS_SCALED(feedrate_mm_s);
	  #if ENABLED(MESH_BED_LEVELING)
		if (mbl.active()) { // direct used of mbl.active() for speed
	  ...
	  #endif
		  line_to_destination_AD(fr_scaled);// HANXU:
	}
  #endif
  return false;
}
...
void prepare_move_to_destination_AD() {
  clamp_to_software_endstops(destination_AD);
  refresh_cmd_timeout();
  ...
  if (
      ...
      prepare_move_to_destination_dualx() || prepare_move_to_destination_cartesian()
    #else
      prepare_move_to_destination_cartesian_AD() // what is running
    #endif
  ) return;

  set_current_to_destination_AD();//HANXU: for H
}
...
void setup() {
  ...
  pinMode(44, OUTPUT);
  analogWrite(44, 0);//Han Xu used for reset lasert to off
