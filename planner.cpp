...
float Planner::max_feedrate_mm_s[XYZE_N], // Max speeds in mm per second
      ...
      Planner::steps_to_mm[XYZE_AD];


float Planner::max_feedrate_mm_s_AD[XYZE_AD], //HANXU:
      Planner::axis_steps_per_mm_AD[XYZE_AD];
...
uint32_t Planner::max_acceleration_mm_per_s2_AD[XYZE_AD]; // HANXU:
...
long Planner::position[XYZE] = { 0 };

long Planner::position_AD[XYZE_AD] = { 0 };//HANXU: for H

uint32_t Planner::cutoff_long;

float Planner::previous_speed[XYZE_AD],
  ...
  // Segment times (in 纰宻). Used for speed calculations
...
void Planner::_buffer_line(const float &a, const float &b, const float &c, const float &e, float fr_mm_s, const uint8_t extruder) {
  ...
  /**
   * Adapted from Pr鏆栨嫥a MKS firmware
...
void Planner::_buffer_line_AD(const float &a, const float &b, const float &c, const float &e, const float &h, float fr_mm_s, const uint8_t extruder) {


  // The target position of the tool in absolute steps
  // Calculate target position in absolute steps
  //this should be done after the wait, because otherwise a M92 code within the gcode disrupts this calculation somehow
  //DEFAULT_AXIS_STEPS_PER_UNIT_AD  { 80, 80, 4000, 500, 100 } //HANXU: the last value will be calibrated later
  const long target_AD[XYZE_AD] = {
    LROUND(a * axis_steps_per_mm_AD[X_AXIS]),
    LROUND(b * axis_steps_per_mm_AD[Y_AXIS]),
    LROUND(c * axis_steps_per_mm_AD[Z_AXIS]),
    LROUND(e * axis_steps_per_mm_AD[E_AXIS]),
    LROUND(h * axis_steps_per_mm_AD[H_AXIS])
  };
  ...
  const long da = target_AD[X_AXIS] - position_AD[X_AXIS],
             db = target_AD[Y_AXIS] - position_AD[Y_AXIS],
             dc = target_AD[Z_AXIS] - position_AD[Z_AXIS],
  			 dh = target_AD[H_AXIS] - position_AD[H_AXIS]; //HANXU: for H
  ...
  long de = target_AD[E_AXIS] - position_AD[E_AXIS]; //HANXU
  ...
  uint8_t dm = 0;
    ...
    if (db < 0) SBI(dm, Y_AXIS);
    if (dc < 0) SBI(dm, Z_AXIS);
	if (dh < 0) SBI(dm, H_AXIS);  //HANXU: direction bit
  #endif
  if (de < 0) SBI(dm, E_AXIS);
  ...
  //Set laser state HANXU:我浪费了一天写了个这个
  block->laser_state = 0;
  ...
  // See http://www.corexy.com/theory.html
    ...
    block->steps[Y_AXIS] = labs(db);
    block->steps[Z_AXIS] = labs(dc);
	block->steps[H_AXIS] = labs(dh);//HANXU
  #endif

  block->steps[E_AXIS] = esteps;
  block->step_event_count = MAX5(block->steps[X_AXIS], block->steps[Y_AXIS], block->steps[Z_AXIS], esteps, block->steps[H_AXIS]);//HANXU
  ...
  //enable active axes
    ...
    if (block->steps[X_AXIS]) enable_X();
    if (block->steps[Y_AXIS]) enable_Y();
	if (block->steps[H_AXIS]) enable_E1();//HANXU:
    #if DISABLED(Z_LATE_ENABLE)
      if (block->steps[Z_AXIS]) enable_Z();
    #endif
  #endif
  ...
  if (esteps) {
    ...
    #else
      enable_E0();
      //enable_E1();  HANXU: dont want rotation to be disturbed by extruder
      enable_E2();
      enable_E3();
      enable_E4();
    #endif
   ...
   */
    ...
    #endif
  #else
    float delta_mm[XYZE_AD];
    delta_mm[X_AXIS] = da * steps_to_mm[X_AXIS];
    delta_mm[Y_AXIS] = db * steps_to_mm[Y_AXIS];
    delta_mm[Z_AXIS] = dc * steps_to_mm[Z_AXIS];
	delta_mm[H_AXIS] = dh * steps_to_mm[H_AXIS];
  #endif
  delta_mm[E_AXIS] = esteps_float * steps_to_mm[E_AXIS_N];

  if (block->steps[X_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[Y_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[Z_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[H_AXIS]==0) {
    block->millimeters = FABS(delta_mm[E_AXIS]);
  }
  else if (block->steps[X_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[Y_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[Z_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[H_AXIS]!=0){
	 block->millimeters = FABS(delta_mm[H_AXIS]); // it is likely wrong, H axis should not affect feedrate
  	}
  else {
    block->millimeters = SQRT(
      #if CORE_IS_XY
  ...
  block->nominal_speed = block->millimeters * inverse_mm_s; // (mm/sec) Always > 0
  block->nominal_rate = CEIL(block->step_event_count * inverse_mm_s); // (step/sec) Always > 0 ceil:round up
  ...
  // Calculate and limit speed in mm/sec for each axis
  float current_speed[XYZE_AD], speed_factor = 1.0; // factor <1 decreases speed
  LOOP_XYZE_AD(i) {
    const float cs = FABS(current_speed[i] = delta_mm[i] * inverse_mm_s);
    #if ENABLED(DISTINCT_E_FACTORS)
      if (i == E_AXIS) i += extruder;
    #endif
    if (cs > max_feedrate_mm_s_AD[i]) NOMORE(speed_factor, max_feedrate_mm_s_AD[i] / cs);
  }
  ...
  // Correct the speed
  if (speed_factor < 1.0) {
    LOOP_XYZE_AD(i) current_speed[i] *= speed_factor;
    block->nominal_speed *= speed_factor;
    block->nominal_rate *= speed_factor;
  }
 ...
 */
void Planner::_set_position_mm_AD(const float &a, const float &b, const float &c, const float &e, const float &h) { //HANXU:
  #if ENABLED(DISTINCT_E_FACTORS)
  ...
  #endif
	  long na = position_AD[X_AXIS] = LROUND(a * axis_steps_per_mm_AD[X_AXIS]),
		   nb = position_AD[Y_AXIS] = LROUND(b * axis_steps_per_mm_AD[Y_AXIS]),
		   nc = position_AD[Z_AXIS] = LROUND(c * axis_steps_per_mm_AD[Z_AXIS]),
		   ne = position_AD[E_AXIS] = LROUND(e * axis_steps_per_mm_AD[_EINDEX]),
  		   nh = position_AD[H_AXIS] = LROUND(h * axis_steps_per_mm_AD[H_AXIS]);
  #if ENABLED(LIN_ADVANCE)
		position_float[X_AXIS] = a;
		position_float[Y_AXIS] = b;
		position_float[Z_AXIS] = c;
		position_float[E_AXIS] = e;
  #endif
	  stepper.set_position_AD(na, nb, nc, ne, nh); //HANXU : we will change it later
	  previous_nominal_speed = 0.0; // Resets planner junction speeds. Assumes start from rest.
	  ZERO(previous_speed);    //HANXU: may need change
}
...
void Planner::set_position_mm_AD(const AxisEnum axis, const float &v) {//HANXU
  #if ENABLED(DISTINCT_E_FACTORS)
    const uint8_t axis_index = axis + (axis == E_AXIS ? active_extruder : 0);
    last_extruder = active_extruder;
  #else
    const uint8_t axis_index = axis;
  #endif
  position_AD[axis] = LROUND(v * axis_steps_per_mm_AD[axis_index]);
  #if ENABLED(LIN_ADVANCE)
    position_float[axis] = v;
  #endif
  stepper.set_position(axis, v);
  previous_speed[axis] = 0.0; //we may change it later
}
...
void Planner::refresh_positioning() {
  LOOP_XYZE_AD(i) steps_to_mm[i] = 1.0 / axis_steps_per_mm_AD[i];
