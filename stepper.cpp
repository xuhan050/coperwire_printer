...
block_t* Stepper::current_block = NULL;  // A pointer to the block currently being traced, executed
...
uint8_t Stepper::last_direction_bits = 0;        // The next stepping-bits to be output, executed
...
long Stepper::counter_X = 0,
     Stepper::counter_Y = 0,
     Stepper::counter_Z = 0,
     Stepper::counter_H = 0, //HANXU
     Stepper::counter_E = 0;  //executed

volatile uint32_t Stepper::step_events_completed = 0; // The number of step events executed in the current block, executed
...
long Stepper::acceleration_time, Stepper::deceleration_time; // executed

volatile long Stepper::count_position[XYZE_AD] = { 0 }; // executed
volatile signed char Stepper::count_direction[XYZE_AD] = { 1, 1, 1, 1,1}; // executed 
...
volatile long Stepper::endstops_trigsteps[XYZ]; //executed
  ...
  #define X_APPLY_STEP(v,Q) X_STEP_WRITE(v) //executed
  #define H_APPLY_STEP(v,Q) H_STEP_WRITE(v)//HANXU
  #define H_APPLY_DIR(v,Q) H_DIR_WRITE(v)
#endif
  ...
  #define Y_APPLY_STEP(v,Q) Y_STEP_WRITE(v) //executed
#endif
  ...
  #define Z_APPLY_STEP(v,Q) Z_STEP_WRITE(v) //executed
#endif
...
#define _ENABLE_ISRs() do { cli(); if (thermalManager.in_temp_isr) CBI(TIMSK0, OCIE0B); else SBI(TIMSK0, OCIE0B); ENABLE_STEPPER_DRIVER_INTERRUPT(); } while(0)//enable isr
...
void Stepper::isr() {
  ...
  #define OCR_VAL_TOLERANCE 1000          // First max delay is 2.0ms, last min delay is 0.5ms, all others 1.5ms // executed
  ...
  #define _SPLIT(L) (ocr_val = (uint16_t)L)
    ...
    if (step_remaining && ENDSTOPS_ENABLED) {   // Just check endstops - not yet time for a step
      ...
      return;
    } // executed define a split for endstop
  #endif
  ...
  if (!current_block) {
    ...
    if (current_block) {
      trapezoid_generator_reset(); // called when new block is generated

      // Initialize Bresenham counters to 1/2 the ceiling
      counter_X = counter_Y = counter_Z = counter_E = -(current_block->step_event_count >> 1);// executed
      ...
      _NEXT_ISR(2000); // Run at slow speed - 1 KHz
      _ENABLE_ISRs(); // re-enable ISRs  //executed if currentblock is still empty after pop up 
      return;
    }
  }
  ...
  #else
    if (ENDSTOPS_ENABLED) endstops.update();// executed
  #endif
  ...
  for (uint8_t i = step_loops; i--;) {
    ...
    #define _COUNTER(AXIS) counter_## AXIS //executed
    #define _APPLY_STEP(AXIS) AXIS ##_APPLY_STEP //executed
    #define _INVERT_STEP_PIN(AXIS) INVERT_## AXIS ##_STEP_PIN //executed
     ...
     * Delays under 20 cycles (1.25纰宻) will be very accurate, using NOPs.
     ...
     * That's every 0.5纰宻 on 16MHz and every 0.4纰宻 on 20MHz.
     * 20 counts of TCNT0 -by itself- is a good pulse delay.
     * 10纰宻 = 160 or 200 cycles.
     */
...
void Stepper::init() { // will look into it 
...
void Stepper::set_position_AD(const long &a, const long &b, const long &c, const long &e,const long &h) {
