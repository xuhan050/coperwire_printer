...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //HANXU: for H this part men be a hiden danger
      const float def1[] = DEFAULT_AXIS_STEPS_PER_UNIT_AD, def2[] = DEFAULT_MAX_FEEDRATE_AD;
      const uint32_t def3[] = DEFAULT_MAX_ACCELERATION_AD;
...
*/
  //HANXU: for H I directly change the code here
  ...
  LOOP_XYZE_N(i) {
    planner.axis_steps_per_mm[i]          = pgm_read_float(&tmp1[i < COUNT(tmp1) ? i : COUNT(tmp1) - 1]);// robust
    planner.max_feedrate_mm_s[i]          = pgm_read_float(&tmp2[i < COUNT(tmp2) ? i : COUNT(tmp2) - 1]);
    planner.max_acceleration_mm_per_s2[i] = pgm_read_dword_near(&tmp3[i < COUNT(tmp3) ? i : COUNT(tmp3) - 1]);
  }

  static const float tmp5[] PROGMEM = DEFAULT_AXIS_STEPS_PER_UNIT_AD, tmp6[] PROGMEM = DEFAULT_MAX_FEEDRATE_AD;
  static const uint32_t tmp7[] PROGMEM = DEFAULT_MAX_ACCELERATION_AD;

  LOOP_XYZE_AD(i) {// HANXU: used to be /*LOOP_XYZE_N(i)*/
    planner.axis_steps_per_mm_AD[i]          = pgm_read_float(&tmp5[i < COUNT(tmp5) ? i : COUNT(tmp5) - 1]);// robust
    planner.max_feedrate_mm_s_AD[i]          = pgm_read_float(&tmp6[i < COUNT(tmp6) ? i : COUNT(tmp6) - 1]);
    planner.max_acceleration_mm_per_s2_AD[i] = pgm_read_dword_near(&tmp7[i < COUNT(tmp7) ? i : COUNT(tmp7) - 1]);
  }
  ...
  //planner.max_jerk[H_AXIS] = DEFAULT_HJERK; //HANXU:for H
